...
when RULE_INIT {
    ...
    set static::debug 1  ;# 0 = production | 1 = QA/debug (verbose security logs)
}
...
when CLIENTSSL_HANDSHAKE {
    ...
    if {[SSL::cipher version] ne "TLSv1.2" && [SSL::cipher version] ne "TLSv1.3"} {
        log local0. "SWAGWAF|TLS_REJECTED|src=[IP::client_addr]|vip=[virtual name]|tls_ver=[SSL::cipher version]"
        reject
    }
}
...
when HTTP_REQUEST {
    ...
    if {[table lookup "block:$ip"] eq "1"} {
        log local0. "SWAGWAF|BLOCKED_REPEAT|src=$ip|xff=$xff|vip=[virtual name]|method=[HTTP::method]|uri=[HTTP::uri]"
    ...
    if {$req_count >= $static::max_requests} {
        ...
        if {$v >= $static::violation_threshold} {
            ...
            log local0. "SWAGWAF|BLOCKED|src=$ip|xff=$xff|vip=[virtual name]|method=[HTTP::method]|uri=[HTTP::uri]|violations=$v"
            HTTP::respond 429 content "{\n  \"error\": \"rate_limit_exceeded\",\n  \"message\": \"Blocked for repeated abuse\",\n  \"retry_after\": 600\n}" "Content-Type" "application/json"
            return
        }
        log local0. "SWAGWAF|RATE_LIMITED|src=$ip|xff=$xff|vip=[virtual name]|method=[HTTP::method]|uri=[HTTP::uri]|req_count=$req_count|violations=$v"
...
when HTTP_REQUEST_DATA {
    ...
    if {$matched_phrase ne ""} {
        ...
        log local0. "SWAGWAF|INJECTION_ATTEMPT|src=$ip|xff=$xff|vip=[virtual name]|method=[HTTP::method]|uri=[HTTP::uri]|phrase=\"$matched_phrase\"|threat=$threat_level"
        ...
        if {$threat_level eq "HIGH"} {
            ...
            if {$v >= $static::violation_threshold} {
                table set "block:$ip" 1 $static::block_seconds
                log local0. "SWAGWAF|BLOCKED|src=$ip|xff=$xff|vip=[virtual name]|violations=$v|reason=injection_threshold"
    ...
    foreach pattern $static::injection_patterns {
        if {[string match -nocase "*$pattern*" $payload_lower]} {
            log local0. "SWAGWAF|INJECTION_ATTEMPT|src=$ip|xff=$xff|vip=[virtual name]|method=[HTTP::method]|uri=[HTTP::uri]|phrase=\"$pattern\"|threat=HIGH|dg=static_fallback"
            set v [table incr "viol:$ip" 3]
            table timeout "viol:$ip" $static::violation_window_ms
            if {$v >= $static::violation_threshold} {
                table set "block:$ip" 1 $static::block_seconds
                log local0. "SWAGWAF|BLOCKED|src=$ip|xff=$xff|vip=[virtual name]|violations=$v|reason=injection_threshold|dg=static_fallback"
                HTTP::respond 403 content "{\n  \"error\": \"forbidden\",\n  \"message\": \"Malicious payload detected\"\n}" "Content-Type" "application/json"
                return
            }
            HTTP::respond 400 content "{\n  \"error\": \"invalid_request\",\n  \"message\": \"Request rejected by security policy\"\n}" "Content-Type" "application/json"
