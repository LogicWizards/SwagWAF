...
when RULE_INIT {
    ...
    # the single source of truth; record values hold owner/service/ticket/expiry
    # metadata for ISA logs and future self-service governance.
    # TLS enforcement, header sanitation, and payload inspection remain active.
    set static::trusted_sources_dg "/Common/dg_swagwaf_trusted_sources"
    if {[catch {class size $static::trusted_sources_dg} trusted_sources_count]} {
        set static::trusted_sources_ready 0
        log local0. "SwagWAF: $static::trusted_sources_dg not deployed — no trusted-source bypass active"
    } else {
...
when HTTP_REQUEST {
    ...
    set trusted_source_policy ""
    if {$static::trusted_sources_ready} {
        set trusted_source_match [class match -name -- $ip equals $static::trusted_sources_dg]
        if {$trusted_source_match ne ""} {
            set trusted_source_policy [class match -value -- $ip equals $static::trusted_sources_dg]
            if {$trusted_source_policy eq ""} {
    ...
    # === RATE LIMITING ===
    if {$trusted_source_match ne ""} {
        # Remove any state created before this IP became trusted. Do not return:
